        ...
        if (newItem) {
            switch (newItem->getClassId()) {
                ...
                case PROTOCOL_CHELL_DATA: {
                    ...
                    auto chell2 = std::make_shared<ChellAnimationView>(newChell->getId(),
                ...
                case PROTOCOL_GATE_DATA: {
                    ...
                    auto gate = std::make_shared<GateView>(gateDTO->getId(), textureFactory.getTextureByName(gate_file_name), renderer);
                ...
                case PROTOCOL_ACID_DATA: {
                    ...
                    auto acid = std::make_shared<AcidView>(textureFactory.getTextureByName(acidAndButtons_filename), renderer);
                ...
                case PROTOCOL_ROCK_BLOCK_DATA: {
                    ...
                    for(int i=0; i<rockBlockDTO->getWidth(); i+=CUT_LEN_BLOCKS) {
                        for (int j = 0; j<rockBlockDTO->getHeight(); j+=CUT_LEN_BLOCKS) {
                            auto rockBlock = std::make_shared<BlockRockView>(textureFactory.getTextureByName(block_file_name), renderer);
                ...
                case PROTOCOL_METAL_BLOCK_DATA: {
                    ...
                    for(int i=0; i<metalBlockDTO->getWidth(); i+=CUT_LEN_BLOCKS) {
                        for (int j = 0; j<metalBlockDTO->getHeight(); j+=CUT_LEN_BLOCKS) {
                            auto metalBlock = std::make_shared<BlockMetalView>(textureFactory.getTextureByName(block_file_name), renderer);
                ...
                case PROTOCOL_METAL_DIAGONAL_BLOCK_DATA: {
                    ...
                    auto diagonalMetalBlock = std::make_shared<DiagonalBlockMetalView>(textureFactory.getTextureByName(block_file_name), renderer, diagonalMetalBlockDTO->getOrientation());
                ...
                case PROTOCOL_ENERGY_TRANSMITTER_DATA: {
                    ...
                    auto energyTransmitter = std::make_shared<EnergyTransmitterView>(energyTransmitterDTO->getId(), textureFactory.getTextureByName(block_file_name), renderer, energyTransmitterDTO->getDirection());
                ...
                case PROTOCOL_ENERGY_RECEIVER_DATA: {
                    ...
                    auto energyReceiver = std::make_shared<EnergyReceiverView>(energyReceiverDTO->getId(), textureFactory.getTextureByName(block_file_name), renderer);
                ...
                case PROTOCOL_ENERGY_BARRIER_DATA: {
                    ...
                    auto energyBarrier = std::make_shared<EnergyBarrierView>(textureFactory.getTextureByName(acidAndButtons_filename), renderer);
                ...
                case PROTOCOL_ROCK_DATA: {
                    ...
                    auto rock = std::make_shared<RockView>(rockDTO->getId(), textureFactory.getTextureByName(bulletAndRock_filename), renderer);
                ...
                case PROTOCOL_CAKE_DATA: {
                    ...
                    auto cake = std::make_shared<CakeView>(textureFactory.getTextureByName(cake_file_name), renderer);
        ...
        while (timeAccumulatedMs >= timeStepMs) {
            ...
            if (newItem) {
                switch (newItem->getClassId()) {
                    case PROTOCOL_CHELL_DATA: {
                        auto newChell = (ChellDTO *) newItem;
                        auto chell2 = std::make_shared<ChellAnimationView>(newChell->getId(), textureFactory.getTextureByName(chell_file_name), renderer);
                    ...
                    case PROTOCOL_ENERGY_BALL_DATA: {
                        ...
                        if (energyBallDTO->getDeleteState() == DELETE) {
                            ...
                            auto ball = std::make_shared<EnergyBallView>(energyBallDTO->getId(), textureFactory.getTextureByName(bulletAndRock_filename), renderer);
